...
public class Registry {
    ...
    public static void registerRecipes(RegistryEvent.Register<IRecipe> event){
        ...
        ItemBase.ITEMS.forEach(item -> {
            ...
            if(!itemRecipes.isEmpty()){
                for(int i = 0; i < itemRecipes.size(); i++){
                    RECIPES.put(new ResourceLocation(Metalworks.MODID, item.getRegistryName().getResourcePath() + "_" + i), itemRecipes.get(i));
                }
            }
        });
        ...
        BlockBase.BLOCKS.forEach(block -> {
            ...
            if(!itemRecipes.isEmpty()){
                for(int i = 0; i < itemRecipes.size(); i++){
                    RECIPES.put(new ResourceLocation(Metalworks.MODID, block.getRegistryName().getResourcePath() + "_" + i), itemRecipes.get(i));
                }
            }
        });
